...
// convertFeatures converts OpenTypeFeature slice to feature tags
// Note: The current version of go-text/typesetting may not support
// explicit feature control in the Input struct
func convertFeatures(features []OpenTypeFeature) []uint32 {
	if len(features) == 0 {
		return nil
	}
	
	result := make([]uint32, len(features))
	for i, f := range features {
		// Convert 4-character tag to uint32
		tag := uint32(0)
		if len(f.Tag) >= 4 {
			tag = uint32(f.Tag[0])<<24 | uint32(f.Tag[1])<<16 | uint32(f.Tag[2])<<8 | uint32(f.Tag[3])
		}
		result[i] = tag
	}
	return result
...
// Helper function to check if a rune is a combining mark
