...
func TestMatrixOperations(t *testing.T) {
	...
	tolerance := 1e-10
	if math.Abs(m.XX) > tolerance || math.Abs(m.YY) > tolerance || 
	   math.Abs(m.XY - (-1.0)) > tolerance || math.Abs(m.YX - 1.0) > tolerance {
...
func TestSurfaceCreation(t *testing.T) {
	...
	if imageSurface, ok := surface.(ImageSurface); ok {
		if imageSurface.GetWidth() != 100 || imageSurface.GetHeight() != 200 {
			t.Errorf("Surface dimensions incorrect, expected 100x200, got %dx%d", 
					imageSurface.GetWidth(), imageSurface.GetHeight())
		}
		
		if imageSurface.GetFormat() != FormatARGB32 {
			t.Errorf("Surface format incorrect, expected %v, got %v", 
					FormatARGB32, imageSurface.GetFormat())
		}
	} else {
...
func TestPatternCreation(t *testing.T) {
	...
	if solidPattern, ok := pattern.(SolidPattern); ok {
		...
		tolerance := 1e-10
		if math.Abs(r - 0.5) > tolerance || math.Abs(g - 0.7) > tolerance || 
		   math.Abs(b - 0.9) > tolerance || math.Abs(a - 0.8) > tolerance {
